...
`Operating System Concepts` 10판, 흔히 말하는 ==공룡책==을 읽으며 정리한 운영체제 글 모음이다. 처음 올린 버전은 키워드 메모에 가까워서, 이번에는 각 장의 문제의식과 대표 개념을 내 기준으로 다시 묶었다.

원문 문장과 삽화는 옮기지 않았다. 설명은 내 말로 다시 썼고, 관계가 헷갈리는 지점은 직접 그린 PNG 다이어그램으로 보강했다.
...
시험이나 면접 준비라면 Ch.03 프로세스, Ch.04 스레드, Ch.05 CPU 스케줄링, Ch.06 동기화, Ch.08 교착 상태, Ch.09-10 메모리 관리, Ch.13-14 파일 시스템이 먼저다. 전체 범위는 Ch.01부터 이어서 읽으면 구조가 잡힌다.

## 시리즈 핵심 요약

- 운영체제는 하드웨어 자원을 직접 노출하지 않고, 프로세스와 파일 같은 안전한 추상화로 감싼다.
- 실행 관리는 프로세스, 스레드, 스케줄링, 동기화, 교착 상태를 한 덩어리로 봐야 한다.
- 메모리, 저장장치, 보안, Linux와 Windows 사례는 앞 개념이 실제 시스템에서 어떻게 만나는지 확인하는 구간이다.
