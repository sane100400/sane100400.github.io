...
> 다이어그램: Windows 커널 모드에는 Executive, Kernel, HAL, 여러 관리자가 계층적으로 놓인다. 객체, 보안, I/O, 메모리 관리가 이 구조 안에서 연결된다.
...
> 다이어그램: 프로세스가 객체를 열면 객체 관리자가 보안 참조 모니터에 검사를 맡기고, 허용된 경우 핸들을 돌려준다. 사용자 코드는 커널 포인터가 아니라 핸들로 객체를 다룬다.
...
21장까지 오면 운영체제의 큰 범위를 한 번 훑은 셈이다. 이후에는 각 장의 연습 문제나 실제 Linux/Windows 관찰로 개념을 굳히면 된다.

## 핵심 요약

- Windows는 사용자 모드와 커널 모드를 나누고, 커널 모드 안에 Executive, Kernel, HAL을 둔다.
- 객체 관리자와 핸들 모델은 파일, 프로세스, 이벤트 같은 커널 자원을 일관되게 다루게 한다.
- 보안 참조 모니터와 NTFS 권한 모델은 Windows 보호 구조의 실제 사례다.
