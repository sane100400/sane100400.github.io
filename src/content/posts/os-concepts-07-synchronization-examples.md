...
공룡책 10판 기준 PDF p.375 근처에서 시작한다. bounded-buffer, readers-writers, dining philosophers 문제부터 동기화 도구를 조합할 때 무엇을 분리해서 봐야 하는지까지 다룬다.
...
3. 버퍼 배열 자체를 수정하는 짧은 구간은 mutex로 보호한다. 빈 칸 수와 배열 보호를 따로 보아야 한다.
