# About Me

![Introduce](/introduce.jpg)

**김현경 (HyeonGyeong Kim)** | Cyber Security

고려대학교 스마트보안학부 / 개인정보보호융합전공 (2022.03 ~ 2026.08 졸업 예정, 학점 3.95)

개인정보보호법부터 해킹 기법까지 폭넓게 학습하며, 정책과 기술을 아우르는 융합적 보안 역량을 키워나가고 있습니다.

---

## Skill Set

### Development

| 구분 | Skill |
|---|---|
| Languages | C, Python, Javascript, Solidity |
| Tooling / DevOps | Github, Docker |
| Environment | Linux, Windows |

### Hacking

| 구분 | Skill |
|---|---|
| Pwnable | Pwntools, GDB 동적 디버깅 |
| Reversing | IDA, Ghidra 정적 분석 / x64dbg, GDB 동적 디버깅 |
| Digital Forensics | Android(ADB), Mac, Windows 아티팩트 수집 및 분석 |
| Blockchain | Solidity 기반 시스템 오디팅 |
| Cryptography | 공개키/비밀키 암호, 블록체인 암호 알고리즘 |
| Web Hacking | Burp Suite 기반 취약점 분석 |

---

## Projects

### Qubic의 Monero Chain에 대한 Selfish Mining 분석
> 2025.09 ~ 2025.11

Monero 네트워크에서 Qubic 마이닝 그룹의 Selfish Mining 행위를 정량적으로 분석. 실제 블록체인 풀에 참여하여 데이터를 수집하고, orphan 블록 패턴 추적, AWS 기반 대시보드 배포, 논문 형식 보고서 작성.

::github{repo="shlee-lab/Qubic-Monero-Selfish-Mining-Analysis"}

### 보안 메신저 통합 평가 프레임워크
> 2025.05 ~ 2025.08

Signal, Telegram, WhatsApp, KakaoTalk, Delta Chat의 보안성-사용성-프라이버시를 N2SF 기반으로 통합 평가. 조직별(정부/기업/민간) 메신저 선택 가이드라인 작성. **한국정보처리학회 논문지 수락 (2025.10)**

### 2024 종합해킹실습 세미나
> 2024.09 ~ 2024.12

고려대학교 학생 대상 주 1회 실습 중심 보안 세미나 총 8회차 진행. 리버싱, 암호학(AES/RSA), 포렌식(Windows FS, Android) 등 강의 자료 자체 제작 및 실습 환경 제공.

### 2024 WiCys CTF 운영
> 2024.02 ~ 2024.03

여성 보안 동아리 WiCys 회장으로서 자체 CTF 대회 기획 및 운영. CTFd 플랫폼을 GCP에 배포하고, CRT/Diffie-Hellman 기반 Crypto 문제 직접 출제.

---

## CTF

| 대회 | 결과 |
|---|---|
| 2025 Dreamhack CTF Season 7 Round #4 | **1위** / 745명 (개인전) |
| 2025 Hacktheon 세종 본선 | 5위 / 9팀 |
| 2025 Hacktheon 세종 예선 | 10위 / 260팀 |
| 2025 KoS CTF (고려 X 성신 연합) | 3위 / 43팀 |
| 2024 SandiegoCTF | 7위 / 670팀 |
| 2024 WolvCTF | 15위 / 625팀 |

---

## Awards & Certifications

- 2024 개인정보보호/보안 정책 아이디어 공모전 (장려)
- 고려대학교 학기최우등생 (2022-1, 2022-2, 2024-2)
- K-Shield Junior 정보보호 관리과정 수료, 프로젝트 1위 (KISA, 2025)
- 네트워크관리사 2급 (2023.10)
- TOEIC 860 (2025.08)

---

## Activities

- 2025 블록체인 누리단 1기 (KISA)
- 2025 개인정보 불법유통 대학생 모니터링단 2기 (KISA)
- 2024 업비트 서포터즈 UPTO! 3기 (두나무)
- 2024 개인정보 불법유통 대학생 모니터링단 (KISA)

---

## Contact

- Email: ruby100400@korea.ac.kr
- GitHub: [github.com/sane100400](https://github.com/sane100400)
