...
공룡책 10판 기준 PDF p.915 근처에서 시작한다. Linux의 역사와 설계 원칙부터 모듈형 단일 커널 구조까지 다룬다.
...
Linux는 모듈을 지원하지만 마이크로커널은 아니다. 중심 서비스는 커널 공간에서 함께 동작하고, 필요한 기능을 loadable kernel module로 확장할 수 있다.
...
> 그림 읽기: copy-on-write fork는 부모와 자식이 처음에는 같은 물리 페이지를 공유하다가 쓰기가 발생한 페이지만 복사한다. 그래서 fork 직후 비용을 크게 줄일 수 있다.
...
Linux의 장치 드라이버는 커널 모듈로 제공되는 경우가 많고, 블록 장치와 문자 장치를 구분해 다룬다. 네트워크 스택은 소켓 인터페이스를 거쳐 사용자 프로그램과 연결되고, 패킷 처리 경로는 드라이버와 프로토콜 계층을 지난다.

보안 측면에서는 사용자와 그룹 권한, capability, namespace, cgroup, LSM 같은 기능이 조합된다. 컨테이너 기술은 이 중 namespace와 cgroup을 적극적으로 사용한다.

## 예시: Linux에서 fork가 가벼워 보이는 이유

1. `fork()` 직후 부모와 자식은 같은 물리 페이지를 공유한다. 둘 중 하나가 쓰기 전까지 실제 복사는 미뤄진다.
2. 쓰기 발생 시 해당 페이지만 복사하므로 명령 실행 준비가 빠르다.
3. 이 copy-on-write 전략은 프로세스 생성, 가상 메모리, 페이지 보호가 함께 맞물린 사례다.
