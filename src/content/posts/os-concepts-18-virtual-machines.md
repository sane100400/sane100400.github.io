...
공룡책 10판 기준 PDF p.838 근처에서 시작한다. ==가상 머신==이 하드웨어 실행 환경을 추상화하는 방식부터 ==Type 1==과 ==Type 2== ==하이퍼바이저==까지 다룬다.
...
가상 머신은 하나의 물리 컴퓨터 위에 여러 개의 독립된 실행 환경을 만든다. 각 ==guest OS==는 자신이 실제 하드웨어를 가진 것처럼 동작하지만, 실제 자원 배분은 하이퍼바이저가 조정한다. 이 구조는 격리, 서버 통합, 테스트, 클라우드 배포에 유용하다.
...
구분의 핵심은 배치 구조다. 어느 쪽이 항상 우월하다고 보기보다, 성능, 관리, 장치 지원, 사용 목적에 따라 선택지가 달라진다고 보는 편이 정확하다.

![가상 머신 - Type 1과 Type 2 하이퍼바이저](/os-concepts/diagrams/os-18-virtual-machines.png)

> 다이어그램: Type 1은 하드웨어 위에 바로 놓이고 Type 2는 호스트 OS 위에서 실행된다. 둘의 차이는 가상 머신을 어느 계층에서 관리하느냐에 있다.
...
> 다이어그램: 게스트가 보는 물리 주소와 호스트의 실제 물리 주소는 다르다. 하이퍼바이저 매핑이 한 층 더 들어가면서 주소 변환 비용도 늘어난다.

## I/O와 운영 기능

I/O 가상화는 장치가 다양하고 성능 요구가 높아 까다롭다. 완전 에뮬레이션은 호환성이 좋지만 느릴 수 있고, paravirtualized driver나 device passthrough는 성능을 높이지만 제약이 생긴다.

스냅샷은 특정 시점의 VM 상태를 저장하고 되돌릴 수 있게 한다. ==Live migration==은 실행 중인 VM을 다른 물리 호스트로 옮겨 유지보수와 부하 분산을 가능하게 한다. 이 기능들은 클라우드 인프라의 기반이다.
...
- 가상 머신은 하드웨어 실행 환경 전체를 소프트웨어로 만들어 guest OS를 격리한다.
- 하이퍼바이저는 CPU, 메모리, I/O 자원을 가상 머신에 나눠 주는 계층이다.
- 스냅샷과 live migration은 클라우드 운영과 테스트 환경에서 VM을 유용하게 만든다.
